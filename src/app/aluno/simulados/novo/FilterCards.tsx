              ...
              <div
                className={`w-5 h-5 rounded border shrink-0 flex items-center justify-center transition-colors duration-300 ${
                  isAllSelected
                    ...
                    : "border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-950"
                }`}
              >
                {isAllSelected && (
          ...
          {filteredItems.map((item: any) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 w-full text-left px-4 py-3 transition-colors duration-300 group ${
                  isSelected
                    ? "bg-gray-50 dark:bg-neutral-950 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-neutral-900"
                    : "text-gray-600 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-900"
                }`}
              >
                ...
                {/* ENVOLTÓRIO FLEXÍVEL PARA O TEXTO E O BADGE */}
                <div className="flex flex-1 items-center justify-between gap-2 overflow-hidden pr-2">
                  <span
                  ...
                  {/* BADGE DE QUANTIDADE DE QUESTÕES AQUI */}
                  {/* Somente exibe se a propriedade existir (e for maior que 0, se desejar) */}
                  {item.quantidadeQuestoes !== undefined && (
                    <span className="shrink-0 text-[9px] font-bold bg-white dark:bg-neutral-800 text-gray-400 dark:text-neutral-400 px-1.5 py-0.5 rounded border border-gray-200 dark:border-neutral-700 group-hover:border-gray-300 dark:group-hover:border-neutral-600 transition-colors">
...
export function FilterCardMaterias({
  ...
  materias = [], // As pastas principais (Matérias)
  assuntos = [], // Os sub-itens (Assuntos)
  materiasSel = {}, // Estado das matérias selecionadas
  assuntosSel = {}, // Estado dos assuntos selecionados
  onToggleMateria,
  onToggleAssunto,
}: any) {
  const [searchTerm, setSearchTerm] = useState("");
  ...
  // Agrupamento: Junta as matérias e coloca os respectivos assuntos dentro delas
  const grouped = useMemo(() => {
    ...
    // Inicializa as pastas com as matérias fornecidas
    materias.forEach((m: any) => {
      groups[m.id] = { id: m.id, nome: m.nome, assuntos: [] };
    });

    // Distribui os assuntos dentro das pastas corretas
    assuntos.forEach((a: any) => {
      const mId = Number(a.materiaId || a.materia_id || 0);
      if (groups[mId]) {
        groups[mId].assuntos.push(a);
      } else if (a.materiaNome) {
        // Fallback caso a relação seja pelo nome da matéria
        const mPai = materias.find((m: any) => m.nome === a.materiaNome);
        if (mPai && groups[mPai.id]) {
          groups[mPai.id].assuntos.push(a);
        }
      }
    });
    ...
    // Aplica o filtro de pesquisa (busca na matéria ou nos assuntos dela)
    if (searchTerm.trim()) {
      const lowerSearch = searchTerm.toLowerCase();
      result = result.filter(
        (g) =>
          g.nome.toLowerCase().includes(lowerSearch) ||
          g.assuntos.some((a: any) =>
  ...
  const totalMateriasMarcadas = Object.keys(materiasSel).length;
  ...
  return (
    <div className="flex flex-col">
      <label className="text-sm font-semibold text-gray-700 dark:text-neutral-300 mb-3 flex justify-between items-center transition-colors duration-300">
        {title}
        <span className="text-[10px] bg-gray-100 dark:bg-neutral-800 px-2 py-0.5 rounded-full text-[#009966] dark:text-emerald-400 transition-colors duration-300">
          {totalMateriasMarcadas} marcadas
        </span>
      </label>
      <div className="flex flex-col h-200 bg-white dark:bg-neutral-950 border border-gray-200 dark:border-neutral-800 rounded-xl overflow-hidden shadow-sm dark:shadow-inner transition-colors duration-300">
        <div className="flex items-center px-4 h-12 shrink-0 border-b border-gray-200 dark:border-neutral-800/60 bg-white dark:bg-neutral-950 transition-colors duration-300">
        ...
        >
          ...
          {grouped.map((data) => {
            ...
            // Se o utilizador estiver a pesquisar, abre as pastas automaticamente
            const isExpanded =
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between w-full px-4 py-3.5 hover:bg-gray-50 dark:hover:bg-neutral-950 transition-colors duration-300">
                  ...
                  <ChevronDown
                    className={`w-4 h-4 text-gray-500 dark:text-neutral-500 transition-transform duration-300 ${
                      isExpanded ? "rotate-180" : ""
                    }`}
                  />
                </div>

                {isExpanded && data.assuntos.length > 0 && (
                  <div className="flex flex-col bg-gray-50 dark:bg-neutral-950 pb-2 transition-colors duration-300">
                    {data.assuntos.map((item: any) => {
                      // Se a matéria (pasta) estiver selecionada, o assunto fica visualmente marcado
                      const isSelected =
                        isFolderSelected || assuntosSel[item.id] !== undefined;
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-3 w-full text-left pr-4 py-2.5 transition-colors duration-300 group ${
                            isSelected
                        ...
                        >
                          <div className="w-12 shrink-0 flex justify-end opacity-40 group-hover:opacity-100 transition-opacity duration-300">
                            <CornerDownRight
                              className={`w-4 h-4 mt-0.5 transition-colors duration-300 ${
                                isSelected
                                  ? "text-[#009966] dark:text-emerald-500"
                          ...
                          <div className="flex flex-1 items-center justify-between gap-2 overflow-hidden pr-2">
                            <span
                              className={`truncate text-[13px] leading-snug transition-colors duration-300 ${
                                isSelected
                                  ? "text-gray-900 dark:text-neutral-200 font-medium"
                                  : "text-gray-500 dark:text-neutral-500"
                              }`}
